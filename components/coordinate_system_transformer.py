...
class CoordinateTransformer:
    def __init__(self) -> None:
        ...
        self.sub_ROSVehiclePoseTransforms = ProtoSubscriber(
            "ROSVehiclePoseTransforms", TFMessage.TFMessage
        )
        self.sub_ROSVLS128CenterCenterRoofTransform = ProtoSubscriber(
            "ROSVLS128CenterCenterRoofTransform", TFMessage.TFMessage
        )

        self.pub_ROSTrafficParticipantListTransformt = ProtoPublisher(
        ...
        self.sub_ROSVehiclePoseTransforms.set_callback(
            self.callback_ROSVehiclePoseTransforms
        )
        self.sub_ROSVLS128CenterCenterRoofTransform.set_callback(
            self.callback_ROSVLS128CenterCenterRoofTransform
        )

    def callback_ROSVehiclePoseTransforms(
        self, topic_name, tfmessage_array_proto_msg, time
    ):
    ...
    def callback_ROSVLS128CenterCenterRoofTransform(
        self, topic_name, tfmessage_array_proto_msg, time
    ):
        if len(tfmessage_array_proto_msg.transforms) > 0:
            for transform in tfmessage_array_proto_msg.transforms:
